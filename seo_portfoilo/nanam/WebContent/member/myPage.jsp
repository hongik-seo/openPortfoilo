...
출처: https://sadtear.tistory.com/106 [[ Be Happy...]]*/
	...
	//비밀번호 비밀번호확인 정규표현식
	var userJoinPwdCheck = new RegExp(/(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,15}$/);
		...
		$("#email").on("keyup",function(){
		 ...
		 $("input[type=password]").on("keyup",function(){	
			if (pwdFind.val() == "") {
				...
				t.eq(3).html("비밀번호(숫자,영문,특수문자 조합 최소 8자 최대10자로 형식을 맞추세요");
			}else if(userJoinPwdCheck.test(pwdFind.val())){
		 ...
		 $("ul#must").on("focusout",function(){
			 
			
			 if(name.val()!="" && id.val()!=""&& $("input[type=password]").val()!="" && phone.val() !=""&& $(".alertP").val()==""){
 ...
 //출처: https://start0.tistory.com/210 [아는 것이 좋은 것이다.]
...
<h2>정보수정</h2>
		...
		<div id="myFormWrap">
			...
			<form name="myForm" id="myForm" method="post" action="/nanam/member/myPageMain">
				<ul id="must">
					<!--이름-->
					<li id="nameWrap" ><label
						for="name">이름 </label> 
						
					<input name="name" value="${login.name}" style="display:none">	  <div>${login.name}</div>
					...
					<li id="idWrap" ><label for="id">아이디</label>
						...
						<div>${login.id}</div>
					<input name="id" value="${login.id}" style="display:none"> <p id="joinIdP" class="alertP" ></p>
					</li>
					
					<li>
					
					<label for="id">포인트</label>
					...
					<li id="pwdWrap" class="inputInterval"><label
						...
						placeholder="(최소 8 자 및 최대 50자, 대문자 하나 이상, 소문자 하나, 숫자 하나 및 특수 문자 하나 이상)" name="pwd"
					...
					<li id="pwdCWrap" ><label
						...
						placeholder="비밀번호(최소 8 자 및 최대 50자, 대문자 하나 이상, 소문자 하나, 숫자 하나 및 특수 문자 하나 이상)" name="pwdFind" required="required"/>
					...
					<li id="tellWrap"><label for="phone">핸드폰 필수사항
						...
						<p id="joinTellP" class="alertP"></p></li>
				</ul>
				<ul id="notMust">		
						<!-- 이메일-->
					<li id="emailWrap" ><label
						...
						<input type="email"
						id="email" class="join notMust"
						placeholder="고객님의 이메일을 입력해주세요" name="email" />
						<p id="joinEmailP" class="alertP"></li>
					<!--// 이메일-->
					<li id="addressWrap" ><label for="userAddress">주소지 선택사항</label><input 
						type="button" onclick="sample6_execDaumPostcode()" value="우편번호 찾기" id="join findAddressBtn" ><br>


						<input type="text" id="sample6_postcode"  placeholder="우편번호" class="join notMust" name="zipcode"> 
						<input type="text" id="sample6_address"  placeholder="주소" class="join notMust" name="address"><br> 
						<input
						type="text" id="sample6_detailAddress" placeholder="상세주소" class="join notMust" name="detailAddress"> 
						<input
						type="text" id="sample6_extraAddress" placeholder="참고항목" class="join notMust" > </li>
